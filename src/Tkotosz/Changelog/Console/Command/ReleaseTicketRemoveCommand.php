...
class ReleaseTicketRemoveCommand extends Command
{
    /**
    ...
    {
        $this->setName('release:ticket:remove')
            ->setDescription('Remove ticket from release')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $release->removeTicket($ticketNumber);

        $this->releaseRepository->save($release);

        $io->success(sprintf('Ticket "%s" successfully removed from the release', $ticketNumber));
    }
}
